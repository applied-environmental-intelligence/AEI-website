...
rings_local = [
    np.stack([r_wire * np.cos(phis), r_wire * np.sin(phis), np.full_like(phis, z)], axis=1)
    for z in z_rings
]

# Longitudinals: always top/bottom plus four around the sides (±30°, ±60° from top)
phi_long = np.deg2rad([-90, 90, 30 ,-30, 0])#, 30, 60])
n_long = phi_long.size
n_z_samples = 110
z_line = np.linspace(+L / 2, -L / 2, n_z_samples)
longs_local = [
    np.stack([np.full_like(z_line, r_wire * np.cos(phi)), np.full_like(z_line, r_wire * np.sin(phi)), z_line], axis=1)
    for phi in phi_long
]

# Cross-section cues at the near end (opening)
front_outer_local = np.stack([r_cable * np.cos(phis), r_cable * np.sin(phis), np.full_like(phis, +L / 2)], axis=1)
...
for i in range(n_frames):
    ...
    if theta == theta_end and p >= blip_start:
        ...
        blip_half_height = r_cable  # same height as cylinder radius (diameter=full height)
        ...
        for zsign in (1.0, -1.0):
            ...
            growth = -1#r_cable * 0.001*(abs(zpos) / max(travel_range, 1e-6))
